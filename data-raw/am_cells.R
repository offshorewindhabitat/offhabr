...
librarian::shelf(
  aquamapsdata, dplyr, DBI, mapview, sf, terra, tibble, units)
devtools::load_all()
...
am_cells_ply <- st_intersection(
  ...
  select(
    cell_id, zone_key,
    hcaf_id = ID, CsquareCode, CenterLong, CenterLat, area_km2, geom)
...
am_cells_grd <- setValues(
  r_cells,
  ifelse(
    values(r_cells) %in% am_cells_ply$hcaf_id,
    values(r_cells),
    NA)) %>%
  trim()
names(am_cells_grd) = "hcaf_id"
am_cells_grd %>%
  writeRaster("inst/am_cells_grd.tif")
